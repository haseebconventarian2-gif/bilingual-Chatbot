...
def select_voice(lang: str) -> str:
    if lang == "ur":
        return os.getenv("AZURE_TTS_VOICE_URDU") or os.getenv("AZURE_TTS_VOICE", "alloy")
...
def create_app() -> FastAPI:
    ...
    async def romanize_urdu(text: str) -> str:
        system = "Transliterate Urdu into Roman Urdu. Output only the transliteration."
        transliterated = await generate_text(user_prompt=text, system_prompt=system, use_tools=False)
        return (transliterated or "").strip()
    ...
    async def unified_message(
        ...
        try:
            ...
            if lang == "ur":
                tts_text = await romanize_urdu(response_text)
    ...
    async def audio_reply(file: UploadFile = File(...)) -> Response:
        ...
        answer, lang = await process_query_with_lang(transcript)
        tts_text = answer
        if lang == "ur":
            tts_text = await romanize_urdu(answer)
    ...
    async def tts(text: str = Query(min_length=1)) -> Response:
        ...
        tts_text = text
        if lang == "ur":
            tts_text = await romanize_urdu(text)
        audio_out = await synthesize_speech(tts_text, voice=select_voice(lang))
        return Response(content=audio_out, media_type=audio_content_type())
